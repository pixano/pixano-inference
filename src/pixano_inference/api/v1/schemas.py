...
"""/v1-specific request/response schemas (camelCase wire contract).

Most capability requests/responses are the camelCase-aliased models in
:mod:`pixano_inference.schemas.inference`. This module adds the two shapes that differ from
the internal model I/O:

* the video-tracking request uses the frontend's nested ``keyframes[].prompts`` layout,
  mapped onto the flat internal :class:`~pixano_inference.models.tracking.TrackingKeyframe`;
* admin/model-management payloads for the deploy/list endpoints.
"""
