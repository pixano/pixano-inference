...
``pycocotools`` (and ``Pillow`` for image inputs) are imported lazily, only inside the
encode/decode helpers and the numeric-array validation branch, so parsing a normal
compressed-bytes response needs neither. Install the ``masks`` extra to use them:
